...
def visual_convs(im, blobs, vis_roi=0, vis_concat=1, vis_kpconv=1, vis_kpdeconv=1):
    dic = {}
    dic['conv3_3'] = np.max(blobs['conv3_3'].data[0], 0)
    dic['conv4_3'] = np.max(blobs['conv4_3'].data[0], 0)
    dic['conv5_3'] = np.max(blobs['conv5_3'].data[0], 0)
    plt.figure(0)
    ax = plt.subplot(2, 2, 1)
    ax.imshow(im[:, :, ::-1], aspect='equal')
    ax.set_title('img')
    ...
    if vis_roi:
        dic['fc_roi_pool3_p14'] = np.max(blobs['fc_roi_pool3_p14'].data[0], 0)
        dic['fc_roi_pool4_p14'] = np.max(blobs['fc_roi_pool4_p14'].data[0], 0)
        dic['fc_roi_pool5_p14'] = np.max(blobs['fc_roi_pool5_p14'].data[0], 0)
        plt.figure(1)
        ax = plt.subplot(2, 2, 1)
        ax.imshow(im[:, :, ::-1], aspect='equal')
        ax.set_title('img')
        ax = plt.subplot(2, 2, 2)
        heatmap = ax.imshow(dic['fc_roi_pool3_p14'], aspect='equal', cmap=plt.cm.jet)
        plt.colorbar(heatmap)
        ax.set_title('fc_roi_pool3_p14')
        ax = plt.subplot(2, 2, 3)
        heatmap = ax.imshow(dic['fc_roi_pool4_p14'], aspect='equal', cmap=plt.cm.jet)
        plt.colorbar(heatmap)
        ax.set_title('fc_roi_pool4_p14')
        ax = plt.subplot(2, 2, 4)
        heatmap = ax.imshow(dic['fc_roi_pool5_p14'], aspect='equal', cmap=plt.cm.jet)
        plt.colorbar(heatmap)
        ax.set_title('fc_roi_pool5_p14')

        if vis_concat:
            dic['fc_roi_pool3_concat_norm'] = np.max(blobs['fc_roi_pool3_concat_norm'].data[0], 0)
            dic['fc_roi_pool4_concat_norm'] = np.max(blobs['fc_roi_pool4_concat_norm'].data[0], 0)
            dic['fc_roi_pool5_concat_norm'] = np.max(blobs['fc_roi_pool5_concat_norm'].data[0], 0)
            dic['fc_concat'] = np.max(blobs['fc_concat'].data[0], 0)
            plt.figure(2)
            ax = plt.subplot(2, 2, 1)
            heatmap = ax.imshow(dic['fc_concat'], aspect='equal', cmap=plt.cm.jet)
            plt.colorbar(heatmap)
            ax.set_title('fc_concat')
            ax = plt.subplot(2, 2, 2)
            heatmap = ax.imshow(dic['fc_roi_pool3_concat_norm'], aspect='equal', cmap=plt.cm.jet)
            plt.colorbar(heatmap)
            ax.set_title('fc_roi_pool3_concat_norm')
            ax = plt.subplot(2, 2, 3)
            heatmap = ax.imshow(dic['fc_roi_pool4_concat_norm'], aspect='equal', cmap=plt.cm.jet)
            plt.colorbar(heatmap)
            ax.set_title('fc_roi_pool4_concat_norm')
            ax = plt.subplot(2, 2, 4)
            heatmap = ax.imshow(dic['fc_roi_pool5_concat_norm'], aspect='equal', cmap=plt.cm.jet)
            plt.colorbar(heatmap)
            ax.set_title('fc_roi_pool5_concat_norm')
        if vis_kpconv:
            dic['kp_conv1'] = np.max(blobs['kp_conv1'].data[0], 0)
            dic['kp_conv2'] = np.max(blobs['kp_conv2'].data[0], 0)
            dic['kp_conv3'] = np.max(blobs['kp_conv3'].data[0], 0)
            dic['kp_conv4'] = np.max(blobs['kp_conv4'].data[0], 0)
            plt.figure(3)
            ax = plt.subplot(2, 2, 1)
            heatmap = ax.imshow(dic['kp_conv1'], aspect='equal', cmap=plt.cm.jet)
            plt.colorbar(heatmap)
            ax.set_title('kp_conv1')
            ax = plt.subplot(2, 2, 2)
            heatmap = ax.imshow(dic['kp_conv2'], aspect='equal', cmap=plt.cm.jet)
            plt.colorbar(heatmap)
            ax.set_title('kp_conv2')
            ax = plt.subplot(2, 2, 3)
            heatmap = ax.imshow(dic['kp_conv3'], aspect='equal', cmap=plt.cm.jet)
            plt.colorbar(heatmap)
            ax.set_title('kp_conv3')
            ax = plt.subplot(2, 2, 4)
            heatmap = ax.imshow(dic['kp_conv4'], aspect='equal', cmap=plt.cm.jet)
            plt.colorbar(heatmap)
            ax.set_title('kp_conv4')
        if vis_kpdeconv:
            dic['kp_deconv1'] = np.max(blobs['kp_deconv1'].data[0], 0)
            dic['kp_deconv2'] = np.max(blobs['kp_deconv2'].data[0], 0)
            dic['kp_deconv3'] = np.max(blobs['kp_deconv3'].data[0], 0)
            dic['kp_score'] = np.max(blobs['kp_score'].data[0], 0)
            plt.figure(4)
            ax = plt.subplot(2, 2, 1)
            heatmap = ax.imshow(dic['kp_deconv1'], aspect='equal', cmap=plt.cm.jet)
            plt.colorbar(heatmap)
            ...
            ax.set_title('kp_deconv3')
            ax = plt.subplot(2, 2, 4)
            heatmap = ax.imshow(dic['kp_score'], aspect='equal', cmap=plt.cm.jet)
            plt.colorbar(heatmap)
            ax.set_title('kp_score')
