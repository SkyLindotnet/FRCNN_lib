...
from fast_rcnn.test import im_detect, im_detect_facePlus, im_detect_facePlus_v1, im_detect_facePlus_by_rois, im_detect_facePlus_v1_by_rois
...
def visual_result_morph(oriImgInfos, detImgInfos, imageSaveDir):
    print 'start visual..'
    mk_dir(imageSaveDir, 1)
    for index, im_path in enumerate(oriImgInfos):
        # visual annotation
...
def det_model_facePlus_v1_pyramid(prototxt, modelPath, modelType, imPaths, DetectFilePath,
    ...
    # parameterPath = '/data5/yyliang/cuda-workspace/py-faster-rcnn_face/py-R-FCN-test/output/threeHusFace/temp.txt'
    # with open(parameterPath, 'w') as f:
    #     for paramName in net.params.keys():
    #         if paramName in ['norm3', 'norm4', 'norm5']:
    #             paramName_w = str(net.params[paramName][0].data.ravel()[0:8])
    #             paramName_b = '[0]'
    #         else:
    #             paramName_w = str(net.params[paramName][0].data.ravel()[0:8])
    #             paramName_b = str(net.params[paramName][1].data.ravel()[0:8])
    #         f.write('%s %s %s\n' % (paramName, paramName_w, paramName_b))
    # exit(1)
...
if __name__ == '__main__':
    ...
    for strategyThresh in np.arange(-0.20, -0.21, -0.01):
        for thresh in np.arange(20000, 30000, 10000):
            for scale in np.arange(-300, 400, 100): # 0.1, 1, 0.1 100000, 0, -10000
                ...
                '''face plus'''
                # method = 'VGG16_faster_rcnn_end2end_with_multianchor_v3-6_fc_fp2_2_iter_100000'
                '''wider face'''
                ...
                method = 'VGG16_faster_rcnn_end2end_with_fuse_multianchor_frozen_v1_lha2_w2-4-1-fc-3-2_1_fp3_m0.5_2_1_s2_iter_70000'

                ''' moon '''
                # method = 'VGG16_faster_rcnn_end2end_with_multianchor_iter_20000_v3'

                modelDir = 'model'
                testDataSet = 'threeHusFace'  # wider FDDB Face_Plus threeHusFace Face_Plus_v1
                # testDataSet = 'moon'
                # modelDir = 'faster_rcnn_alt_opt/v3/s11'  # s2 s4 s5 s7 s8 s13
                methodType = 'frozen'  # rpn normal frozen
                ''' wider face '''
                ...
                # generateFaceDB('S3', prototxt, modelPath, methodType, CONF_THRESH=CONF_THRESH,
                #                NMS_THRESH=NMS_THRESH, DBtype='morph', recompute=1,
                #                saveDIR=saveDIR, visual=1, SplitType='X')
                # print 'done'
                #
                # # main run
                timer = Timer()
