...
def mulsrc_lbl_img_path_converter(path, version):
    mulsrc_setnames = ['afw', 'helen', 'ibug', 'lfpw', 'morph','frgc']
    check = False
...
class FaceImage():
    ...
    def vis_diy(self,showfig=True):
        ...
        boxes = self.get_bboxes('xyxy')
        kps = self.get_keypoints()
        ##draw_bbox(ax, boxes, color='cyan')
        for kp in kps:
            if kp is None: continue
            plot(kp[:, 0], kp[:, 1], 'g.', color=(0,1,0),ms=__get_lm_size(kp), alpha=0.8)
    ...
    def is_complete(self):
        #return complete num, all_num
        ages = self.get_ages()
        gender = self.get_genders()
        race = self.get_ethnicity()
        kpt = self.get_keypoints()

        num_faces = len(ages)
        bitmap = [True for _ in range(num_faces)]

        for i in range(num_faces):
            if ages[i] is None: bitmap[i] = False
        for i in range(num_faces):
            if gender[i] is None: bitmap[i] = False
        for i in range(num_faces):
            if race[i] is None: bitmap[i] = False
        for i in range(num_faces):
            if kpt[i] is None: bitmap[i] = False

        T = [i for i in bitmap if i == True]
        return bitmap
...
if __name__ == '__main__':
    ...
    img_list, version_list = get_latest_img_list(['facepp_train', 'facepp_val', 'facepp_test'])
