...
class ProposalLayer(caffe.Layer):
    ...
    def setup(self, bottom, top):
        ...
        self._anchors = generate_anchors(base_size=self._feat_stride, scales=np.array(cfg.TRAIN.MULTI_SCALE_RPN_SCALE[anchor_scale]),
                                         ratios=cfg.GENERATION_ANCHOR_RATIOS)
    ...
    def forward(self, bottom, top):
        ...
        all_scores = all_scores_rpn.reshape((self._ndim, -1, 1))

        all_bbox_deltas_rpn = bottom[1].data.transpose((0, 2, 3, 1)).reshape((-1, 4))
