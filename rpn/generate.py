...
import fast_rcnn.train  # for call generate.py in train.py
...
def _vis_rpn_rois(boxes, scores):
    pass
...
def im_proposals(net, im):
    ...
    boxes = net.blobs['rois'].data[:, 1:].copy() / scale  # blobs_out
    scores = net.blobs['rpn_scores'].data.copy()
    if 1:
        dets = np.hstack((boxes, scores))
        # from IPython import embed; embed()
        _vis_proposals(im, dets[:300, :], thresh=0.5)  # 3 0.9
        plt.show()
    return boxes, scores
...
def imdb_proposals(net, imdb):
    ...
    for i in xrange(imdb.num_images):
        ...
        if 1:
            ...
            # from IPython import embed; embed()
            _vis_proposals(im, dets[:500, :], thresh=0.5)  # 3 0.9
            plt.show()
...
def imdb_rpn_compute_stats(net, imdb, anchor_scales=(8,16,32),
    ...
    squred_sums = 0
    counts = 0
    roidb = fast_rcnn.train.filter_roidb(imdb.roidb)
