...
class AnchorTargetLayer(caffe.Layer):
    ...
    def debugAnchorPosition(self, shifts, im_info, gt_boxes, valid_regions, height, width):
        ...
        # anchor_scales_v4 =
        anchor_scales_v1 = (1, 2)
        anchor_scales_v2 = (4, 8)
        anchor_scales_v3 = (16, 32)
    ...
    def debugRPNAnchorPosition(self, shifts, im_info, gt_boxes, valid_regions, height, width, anchor_scales, feat_stride, rpnTargetAnchors):
        if cfg.TRAIN.VISUAL_ANCHORS_IMG != '':
            plt.figure(0)
            ax = plt.subplot(1, 2, 1)
            im = cv2.imread(cfg.TRAIN.VISUAL_ANCHORS_IMG)
        ...
        for i, anchor_scales in enumerate(anchor_scales_list):
            ...
            scaleList = np.array([0.5, 1, 2, 4, 8, 16, 32])  # np.array(cfg.DEFALU_ANCHOR_SCALES)
            colors = ['b', 'g', 'c', 'm', 'y', 'k', 'b']
            target_anchors = anchors[np.where(max_overlaps > cfg.TRAIN.MULTI_SCALE_RPN_POSITIVE_OVERLAP)]
            target_anchors = np.vstack((anchors[gt_argmax_overlaps], target_anchors))
            for anchor in target_anchors:
                anchor_w = anchor[2] - anchor[0]
                anchor_h = anchor[3] - anchor[1]
            ...
            for gt_boxe in gt_boxes:
                ...
                ax.add_patch(rec)
            # print 'num of target anchor(>0.5): %s' % target_anchors.shape[0]
            plt.title(str(anchor_scales)+'\nnum of target anchor(default): %s' % target_anchors.shape[0], fontsize=12)
        plt.tight_layout()
        ...
        for i, anchor_scales in enumerate(anchor_scales_list):
            ...
            for gt_boxe in gt_boxes:
                ...
                ax.add_patch(rec)
            # print 'num of target anchor(>0.5): %s' % target_anchors.shape[0]
            plt.title(str(anchor_scales)+'\nnum of target anchor(exclude un-matched gt): %s' % target_anchors.shape[0], fontsize=12)
        plt.tight_layout()
        ...
        colors = ['b', 'g', 'c', 'm', 'y', 'k', 'b']
        # target_anchors = anchors[np.where(max_overlaps > cfg.TRAIN.MULTI_SCALE_RPN_POSITIVE_OVERLAP)]
        for anchor in rpnTargetAnchors:
            anchor_w = anchor[2] - anchor[0]
            anchor_h = anchor[3] - anchor[1]
            scale = int(round(math.sqrt(anchor_w*anchor_h/(feat_stride*feat_stride))))
            index = np.where(scaleList == scale)[0]
        ...
        for gt_boxe in gt_boxes:
            ...
            ax.add_patch(rec)
        # print 'num of target anchor(>0.5): %s' % target_anchors.shape[0]
        plt.title(str(anchor_scales)+'\nnum of target anchor(include un-matched gt adaptively): %s' % target_anchors.shape[0], fontsize=12)
        plt.tight_layout()
    ...
    def setup(self, bottom, top):
        if cfg.TRAIN.MULTI_SCALE_RPN == 1:
            ...
            self._anchor_scales = cfg.TRAIN.MULTI_SCALE_RPN_SCALE
            for i, ANCHOR_SCALES in enumerate(cfg.TRAIN.MULTI_SCALE_RPN_SCALE):
                ...
                # labels
                top[0+i*4].reshape(1, 1, A * height, width)
                # bbox_targets
                top[1+i*4].reshape(1, A * 4, height, width)
                # bbox_inside_weights
                top[2+i*4].reshape(1, A * 4, height, width)
                # bbox_outside_weights
                top[3+i*4].reshape(1, A * 4, height, width)
        else:
            layer_params = yaml.load(self.param_str)
    ...
    def forward(self, bottom, top):
        ...
        if cfg.TRAIN.MULTI_SCALE_RPN == 1:
            ...
            for i, ANCHOR_SCALES in enumerate(cfg.TRAIN.MULTI_SCALE_RPN_SCALE):
                # map of shape (..., H, W)
                height_a, width_a = bottom[i].data.shape[-2:]
                height.append(height_a)
                width.append(width_a)
                # 1. Generate proposals from bbox deltas and shifted anchors
                shift_x = np.arange(0, width[i]) * self._feat_stride[i]
                shift_y = np.arange(0, height[i]) * self._feat_stride[i]
                shift_x, shift_y = np.meshgrid(shift_x, shift_y)
                shifts_a = np.vstack((shift_x.ravel(), shift_y.ravel(),
                                    shift_x.ravel(), shift_y.ravel())).transpose()
                shifts.append(shifts_a)
                # add A anchors (1, A, 4) to
                # cell K shifts (K, 1, 4) to get
                # shift anchors (K, A, 4)
                # reshape to (K*A, 4) shifted anchors
                A_a = self._num_anchors[i]
                A.append(A_a)
                K = shifts[i].shape[0]  # number of proposals is width*height
                all_anchors_a = (self._anchors[i].reshape((1, A[i], 4)) +
                               shifts[i].reshape((1, K, 4)).transpose((1, 0, 2)))
                all_anchors_a = all_anchors_a.reshape((K * A[i], 4))
                all_anchors.append(all_anchors_a)
                total_anchors_a = int(K * A[i])
                total_anchors.append(total_anchors_a)

                # only keep anchors inside the image
                inds_inside_a = np.where(
                    (all_anchors[i][:, 0] >= -self._allowed_border) &
                    (all_anchors[i][:, 1] >= -self._allowed_border) &
                    (all_anchors[i][:, 2] < im_info[1] + self._allowed_border) &  # width
                    (all_anchors[i][:, 3] < im_info[0] + self._allowed_border)    # height
                )[0]

                # if anchor is null, adjust allowed_border
                if len(inds_inside_a) == 0:
                    inds_inside_a = np.where(
                    (all_anchors[i][:, 0] >= -9999) &
                    (all_anchors[i][:, 1] >= -9999) &
                    (all_anchors[i][:, 2] < im_info[1] + 9999) &  # width # height
                    (all_anchors[i][:, 3] < im_info[0] + 9999)
                    )[0]

                inds_inside.append(inds_inside_a)

                # keep only inside anchors
                anchors_a = all_anchors[i][inds_inside[i], :]
                anchors.append(anchors_a)

                # label: 1 is positive, 0 is negative, -1 is dont care
                labels_a = np.empty((len(inds_inside[i]), ), dtype=np.float32)
                labels_a.fill(-1)
                ...
                overlaps_a = bbox_overlaps(
                    np.ascontiguousarray(anchors[i], dtype=np.float),
                    np.ascontiguousarray(gt_boxes, dtype=np.float))
                # if len(overlaps_a) == 0:
                #     print 'anchors[i]\n'
                #     print anchors[i]
                ...
                overlaps.append(overlaps_a)

                #region extend setting
                if cfg.TRAIN.EXTEND_ANCHORS_SELECTING:
                    ...
                    # else:
                    valid_regions = _compute_valid_regions_extend(height, width, gt_boxes, self._feat_stride[i], 0)
                ...
                argmax_overlaps_a = overlaps[i].argmax(axis=1)
                max_overlaps_a = overlaps[i][np.arange(len(inds_inside[i])), argmax_overlaps_a]
                gt_argmax_overlaps_a = overlaps[i].argmax(axis=0)
                gt_max_overlaps_a = overlaps[i][gt_argmax_overlaps_a,
                                           np.arange(overlaps[i].shape[1])]
                gt_argmax_overlaps_a = np.where(overlaps[i] == gt_max_overlaps_a)[0]
                argmax_overlaps.append(argmax_overlaps_a)
                max_overlaps.append(max_overlaps_a)
                gt_argmax_overlaps.append(gt_argmax_overlaps_a)
                gt_max_overlaps.append(gt_max_overlaps_a)

            # anchor with MaxOverlap select
            if cfg.TRAIN.MULTI_SCALE_RPN_SELECT == 2:
                compareIndex = []
                compareMaxOverlap = []
                for k, gt_max_overlap in enumerate(gt_max_overlaps):
                    if compareMaxOverlap == [] and compareIndex == []:
                        compareIndex = np.array([k]*len(gt_max_overlap))
                        compareMaxOverlap = np.array(gt_max_overlap)
                    else:
                        compareResult = compareMaxOverlap > gt_max_overlap
                        newIndex = np.where(compareResult == False)[0]
                        compareIndex[newIndex] = k
                        compareMaxOverlap[newIndex] = np.array(gt_max_overlap)[newIndex]
            ...
            for i, ANCHOR_SCALES in enumerate(cfg.TRAIN.MULTI_SCALE_RPN_SCALE):
                ...
                if cfg.TRAIN.EXTEND_ANCHORS_SELECTING:
                    labels[i][target_inds] = 1  # undo
                else:
                    if cfg.TRAIN.MULTI_SCALE_RPN_SELECT == 0:
                        # fg label: for each gt, anchor with highest overlap
                        labels[i][gt_argmax_overlaps[i]] = 1
                        # fg label: above threshold IOU
                        labels[i][max_overlaps[i] >= cfg.TRAIN.MULTI_SCALE_RPN_POSITIVE_OVERLAP] = 1
                    elif cfg.TRAIN.MULTI_SCALE_RPN_SELECT == 1:
                        labels[i][max_overlaps[i] >= cfg.TRAIN.MULTI_SCALE_RPN_POSITIVE_OVERLAP] = 1
                    elif cfg.TRAIN.MULTI_SCALE_RPN_SELECT == 2:
                        targetIndex = np.where(compareIndex == i)[0]
                        targetMaxOverlap = compareMaxOverlap[targetIndex]
                        gt_argmax_overlaps[i] = np.where(overlaps[i][:, targetIndex] == targetMaxOverlap)[0]
                        labels[i][gt_argmax_overlaps[i]] = 1
                        labels[i][max_overlaps[i] >= cfg.TRAIN.MULTI_SCALE_RPN_POSITIVE_OVERLAP] = 1
                ...
                if cfg.TRAIN.VISUAL_ANCHORS:
                    ...
                    # else:
                    valid_regions = _compute_valid_regions_extend(height[i], width[i], gt_boxes, self._feat_stride[i], 0)
                    targetAnchors = np.vstack((anchors[i][gt_argmax_overlaps[i]], anchors[i][max_overlaps[i] >= cfg.TRAIN.MULTI_SCALE_RPN_POSITIVE_OVERLAP]))
                    # targetAnchors = anchors[i][max_overlaps[i] >= cfg.TRAIN.MULTI_SCALE_RPN_POSITIVE_OVERLAP]
                    self.debugRPNAnchorPosition(shifts[i], im_info, gt_boxes, valid_regions, height[i], width[i],
                                                self._anchor_scales[i], self._feat_stride[i], targetAnchors)

                if cfg.TRAIN.RPN_CLOBBER_POSITIVES:
                    # assign bg labels last so that negative labels can clobber positives
                    labels[i][max_overlaps[i] < cfg.TRAIN.MULTI_SCALE_RPN_NEGATIVE_OVERLAP] = 0

                # undo
                if cfg.TRAIN.RPN_POSITION_ANCHORS_OHEM:
                    # from softmax layer

                    all_fg_scores = bottom[0].data[:, self._num_anchors[i]:, :, :]
                    ...
                    if cfg.TRAIN.RPN_POSITION_ANCHORS_OHEM_MINING_BG:
                        ...
                        if cfg.TRAIN.RPN_POSITION_ANCHORS_OHEM_MINING_BALANCE == 1:
                            ...
                            if len(bg_inds) > bg_num:
                                ...
                                num_of_position = (bg_num + (self._num_anchors[i] - bg_num % self._num_anchors[i])) / self._num_anchors[i]
                                valid_bg_position_inds = np.array([np.where(sort_valid_bg_position == i)[0][:num_of_position] for i in range(1, self._num_anchors[i]+1)]).ravel()
                                ...
                                assert len(valid_bg_inds) == self._num_anchors[i] * num_of_position
                        else:
                    ...
                    fg_inds = np.where(labels[i] == 1)[0]
                    if cfg.TRAIN.RPN_POSITION_ANCHORS_NUM_ADAPT:
                        fg_inds_num = len(np.where([i] == 1)[0])
                        num_bg = fg_inds_num
                        bg_inds = bg_inds
                ...
                bbox_targets = np.zeros((len(inds_inside[i]), 4), dtype=np.float32)
                bbox_targets = _compute_targets(anchors[i], gt_boxes[argmax_overlaps[i], :])

                bbox_inside_weights = np.zeros((len(inds_inside[i]), 4), dtype=np.float32)
                bbox_inside_weights[labels[i] == 1, :] = np.array(cfg.TRAIN.RPN_BBOX_INSIDE_WEIGHTS)
                bbox_outside_weights = np.zeros((len(inds_inside[i]), 4), dtype=np.float32)
                if cfg.TRAIN.RPN_POSITIVE_WEIGHT < 0:
                    ...
                    negative_weights = ((1.0 - cfg.TRAIN.RPN_POSITIVE_WEIGHT) /
                                        np.sum(labels[i] == 0))
                bbox_outside_weights[labels[i] == 1, :] = positive_weights
                bbox_outside_weights[labels[i] == 0, :] = negative_weights

                # map up to original set of anchors
                labels[i] = _unmap(labels[i], total_anchors[i], inds_inside[i], fill=-1)
                bbox_targets = _unmap(bbox_targets, total_anchors[i], inds_inside[i], fill=0)
                bbox_inside_weights = _unmap(bbox_inside_weights, total_anchors[i], inds_inside[i], fill=0)
                bbox_outside_weights = _unmap(bbox_outside_weights, total_anchors[i], inds_inside[i], fill=0)

                # labels
                labels[i] = labels[i].reshape((1, height[i], width[i], A[i])).transpose(0, 3, 1, 2)
                labels[i] = labels[i].reshape((1, 1, A[i] * height[i], width[i]))
                top[0+i*4].reshape(*labels[i].shape)
                top[0+i*4].data[...] = labels[i]

                # bbox_targets
                bbox_targets = bbox_targets \
                    .reshape((1, height[i], width[i], A[i] * 4)).transpose(0, 3, 1, 2)
                top[1+i*4].reshape(*bbox_targets.shape)
                top[1+i*4].data[...] = bbox_targets

                # bbox_inside_weights
                bbox_inside_weights = bbox_inside_weights \
                    .reshape((1, height[i], width[i], A[i] * 4)).transpose(0, 3, 1, 2)
                assert bbox_inside_weights.shape[2] == height[i]
                assert bbox_inside_weights.shape[3] == width[i]
                top[2+i*4].reshape(*bbox_inside_weights.shape)
                top[2+i*4].data[...] = bbox_inside_weights

                # bbox_outside_weights
                bbox_outside_weights = bbox_outside_weights \
                    .reshape((1, height[i], width[i], A[i] * 4)).transpose(0, 3, 1, 2)
                assert bbox_outside_weights.shape[2] == height[i]
                assert bbox_outside_weights.shape[3] == width[i]
                top[3+i*4].reshape(*bbox_outside_weights.shape)
                top[3+i*4].data[...] = bbox_outside_weights
        else:
            # map of shape (..., H, W)
            height, width = bottom[0].data.shape[-2:]
            # GT boxes (x1, y1, x2, y2, label)
            ...
            if cfg.TRAIN.RPN_KP_REGRESSION:
                ...
                keyPoints_targets = _unmap(keyPoints_targets, len(inds_inside), fgInds, fill=0)
                ...
                # timer.toc()
                # print ('anchors 1 took {:.3f}s').format(timer.total_time)
        ...
        if cfg.TRAIN.RPN_KP_REGRESSION:
            ...
            # timer.toc()
            # print ('anchors 2 took {:.3f}s').format(timer.total_time)
    ...
    def forward_temp(self, bottom, top):
        # Algorithm:
        #
        ...
        if cfg.TRAIN.MULTI_SCALE_RPN == 1:
            ...
            for i, ANCHOR_SCALES in enumerate(cfg.TRAIN.MULTI_SCALE_RPN_SCALE):
                ...
                A = self._num_anchors[i]
                K = shifts.shape[0]  # number of proposals is width*height
                all_anchors = (self._anchors[i].reshape((1, A, 4)) +
                               shifts.reshape((1, K, 4)).transpose((1, 0, 2)))
                all_anchors = all_anchors.reshape((K * A, 4))
                total_anchors = int(K * A)
                ...
                if cfg.TRAIN.VISUAL_ANCHORS:
                    ...
                    valid_regions = _compute_valid_regions_extend(height, width, gt_boxes, self._feat_stride[i], 0)
                    self.debugPyramidAnchorPosition(shifts, im_info, gt_boxes, valid_regions,
                                                        height, width, self._anchor_scales[i], self._feat_stride[i])
                # add scale info for visual attribute in proposal target layer
                if cfg.TRAIN.VISUAL_ATTRIBUTES:
                    cfg.TRAIN.VISUAL_SCALE = im_info[2]
                ...
                labels = labels.reshape((1, height, width, A)).transpose(0, 3, 1, 2)
                labels = labels.reshape((1, 1, A * height, width))
                top[0+i*4].reshape(*labels.shape)
                top[0+i*4].data[...] = labels
